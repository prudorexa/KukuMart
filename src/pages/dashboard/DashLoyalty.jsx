...
const TIERS = [
  { key:"bronze",   label:"Bronze",   min:0,    max:199,  color:"bg-orange-100 text-orange-800",  ring:"ring-orange-300",  icon:"🥉" },
  { key:"silver",   label:"Silver",   min:200,  max:499,  color:"bg-gray-100 text-gray-700",       ring:"ring-gray-300",    icon:"🥈" },
  { key:"gold",     label:"Gold",     min:500,  max:999,  color:"bg-amber-100 text-amber-800",     ring:"ring-amber-400",   icon:"🥇" },
  { key:"platinum", label:"Platinum", min:1000, max:99999,color:"bg-purple-100 text-purple-800",   ring:"ring-purple-400",  icon:"💎" },
];
...
export default function DashLoyalty() {
  ...
  async function handleRedeem(reward) {
    ...
    const [, logRes] = await Promise.all([
      supabase.from("profiles").update({ loyalty_points: points - reward.cost }).eq("id", user.id),
      supabase.from("loyalty_history").insert([{
        user_id: user.id, points: -reward.cost,
    ...
    if (!logRes.error) {
      ...
      const { data: hist } = await supabase.from("loyalty_history").select("*").eq("user_id",user.id).order("created_at",{ascending:false}).limit(10);
      setHistory(hist??[]);
  ...
  return (
    <div className="flex flex-col gap-5">
      {/* Toast */}
      {toast && (
        <div className={`rounded-2xl p-4 text-sm font-medium border ${toast.type==="error" ? "bg-red-50 border-red-200 text-red-800" : "bg-green-50 border-green-200 text-green-800"}`}>
          {toast.msg}
        </div>
      )}
      ...
      <div className="bg-[#C8290A] rounded-2xl p-6 text-white relative overflow-hidden">
        ...
        <div className="flex items-start justify-between mb-4 relative z-10">
          ...
          <span className={`text-sm font-bold px-3 py-1.5 rounded-full ${tier.color} shrink-0`}>
        ...
        {!nextTier && (
          <p className="text-red-200 text-xs relative z-10">🏆 You've reached the highest tier — Platinum!</p>
        )}
      </div>
            ...
            return (
              <div key={reward.id} className={`bg-white border rounded-2xl p-4 flex items-center gap-4 transition-all ${
                ...
                <div className="flex flex-col items-end gap-1.5 shrink-0">
                  ...
                  >
                    {redeeming === reward.id ? "..." : canAfford ? "Redeem" : `Need ${reward.cost - points} more`}
                  </button>
                </div>
              </div>
            );
        ...
        <div className="flex flex-col gap-3">
          {EARN_RULES.map(r => (
            <div key={r.label} className="flex items-center gap-3">
              <span className="text-base w-6 shrink-0">{r.icon}</span>
      ...
      {!loading && history.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider px-4 py-3 border-b border-gray-100">Recent activity</p>
          <div className="divide-y divide-gray-50">
            {history.map((h, i) => (
              <div key={i} className="flex items-center justify-between px-4 py-3">
                <p className="text-xs text-gray-700 flex-1">{h.description}</p>
